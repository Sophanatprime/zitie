...
\def\zitieversion{1.2}
\def\zitiedate{农历二〇二一年八月十八}
...
\date{\texorpdfstring{\framezi*[width=2\ccwd,font=文征明行草简繁]{\zitiedate}}{\zitiedate} v\zitieversion}
\author{陈文建\texttt{longaster@163.com}}
...
本宏包将加载 \pkg{xeCJK} 宏包\cite{pkg-xeCJK}。
...
\begin{function}{\zitienewfont,\zitieCJKfamily}
  \begin{syntax}
    ...
    \cs{zitieCJKfamily}- \marg{font family}
  \end{syntax}
  这个命令用来加载和设置需要的字体，\meta{font family} 为在 \cs{framezi} 等命令的 \opt{font} 选项中使用的名字，也可通过 \cs{zitieCJKfamily} 来在文档中使用该字体。\meta{font name} 为字体的名字，\meta{features}，为 \pkg{fontspec} 宏包\cite{pkg-fontspec} 和 \pkg{xeCJK} 宏包支持的字体特性。
...
\begin{function}{\framezi,\framerange,\framezifile}
  ...
  \meta{逗号分隔的16进制序列} 应使用如下的形式：\verb|"4E00 -> "4E27, "4E30| 或 \verb|`一 -> `丢, `丰|，与其它设置字符序列的宏包相似（如 \pkg{xeCJK} 宏包的 \cs{xeCJKDeclareSubCJKBlock}）。注意，由于网格化操作需要大量的计算，因此单个 \meta{range} 不应太大，依据编译环境而定，一般不应超过 2000 个，否则可能无法成功编译。其它命令中也是如此。
...
\begin{function}{basechar,fontsize,zihao}
  \begin{syntax}
    ...
    zihao = \meta{字号}
  \end{syntax}
  \opt{basechar} 设置当 \opt{resize} 为 \opt{base} 类时的基字符，这个用来计算缩放比例，基字符不同时，即使给定相同的缩放比例，其实际缩放比例也可能不同。可使用后文介绍的 \cs{zitiebasechar} 来引用当前的 \opt{basechar} 值。

  \opt{fontsize} 设置计算基字符时的字大小，默认值也为 \tn{normalsize}。\opt{zihao} 设置计算缩放比例时的字号大小，必须加载 \pkg{ctexsize} 或 \pkg{ctex} 宏包才可使用。

  \opt{basechar} 和 \opt{fontsize} 应使用 \cs{zitiesetup} 或宏包选项全局地设置，在 \cs{framezi} 中使用该选项，则该修改在之后的 \cs{framezi} 中并不会生效，即设置是局部的。

  当修改字体大小时，若要在 \cs{framezi} 等命令的外部使用其宽高寄存器（\tn{zitiebasecharwidth}、\tn{zitiebasecharheight}），则应重新设置 \opt{basechar} 或 \opt{fontsize}。\cs{framezi} 等命令将自动重新计算，但不会改变全局的值。
\end{function}
...
\begin{function}{frametype,resize}
  ...
  \end{syntax}
  \opt{frametype} 设置方框样式。可用值的效果正如选项值：\verb|口|--仅方框，\verb|十|--仅中间的横线和竖线，\verb|田|--常见的田字格，\verb|米|--十字格再加上斜的两条线，\verb|咪|--常见的米字格。还可自定义方框，详见第 \ref{sec:interface} 节。以下 \opt{frametype} 分别为 \verb|none|，\verb|口|，\verb|十|，\verb|田|，\verb|米|，\verb|咪|。
...
\begin{function}{
  ...
  \begin{syntax}
    charstroke = <none|solid|dashed|whitesolid|whitedashed|invisible|...> \init{none}
  ...
  \opt{whitedashed} 在 \opt{solid} 和 \opt{dashed} 的基础上将字填充为白色。\opt{invisible} 将字符设置为不可见，但不影响背景和网格的显示，隐藏的字仍然可被复制。
...
\begin{function}{
  ...
  \end{syntax}
  \cs{zitiesetup} 为 \pkg{zitie} 宏包的设置命令，这个命令设置的值在当前组中有效。\cs{zitiecolorlet} 定义新的颜色。注意，若加载 \pkg{xcolor} 宏包，使用 \tn{colorlet} 命令定义的颜色无法在 \pkg{zitie} 宏包中使用，必须使用 \cs{zitiecolorlet} 来定义颜色。
\end{function}
...
\begin{verbatim}
  ...
  \zitienewrule {charstroke} {invisible} { \zitiestrokechars{3 Tr} {#1} }
\end{verbatim}
定义了五个 \opt{charstroke} 处理规则，这样就可以使用 \verb|charstroke=solid, ...| 来处理字符。

\changes{v1.2}{2021/09/23}{新增 \cs{zitiebasechar}、\cs{zitiebasecharwidth}、\cs{zitiebasecharheight}}
\begin{function}{
  \zitiebasechar,
  \zitiebasecharwidth,
  \zitiebasecharheight,
  ...
  \cs{zitiebasechar}、\cs{zitiebasecharwidth}、\cs{zitiebasecharheight} 是局部设置的。其它的为全局设置的接口。
\end{function}
...
\begin{function}{
  ...
  这些是全局保存的颜色名，可在 \pkg{xcolor} 宏包的 \tn{color} 命令 和 \LaTeX3 命令 \cs{color_select:n} 及 \cs{zitie_color_select:n} 中使用。
\end{function}
...
\pkg{background} 的键值也可通过 \cs{zitiesetup}\verb|{background=|\marg{background key-val}\verb|}| 来设置。
